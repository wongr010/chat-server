...
int INVITE(struct user *userbase, char *username, int sessionID){
	int i;
...
int JN_ACK(int sockfd){
	char buffer[MAX_LEN];
	bzero(buffer,sizeof(char)*MAX_LEN);
	sprintf(buffer,"%s:5:","server");
	return send(sockfd, buffer, MAXBUFLEN, 0);
...
int MESSAGE(struct user* userbase, char *message, int sockfd){
	...
	for (i=0; i<SIZE_QUEUE; i++){
		...
		int err=send(userbase[i].sockfd, message, MAXBUFLEN, 0);
			...
			for (i =base_socket; i<= max_sockfd; i ++){
				if (FD_ISSET(i,&copy)){//Implies new data, we need to parse
					...
					case 13: //invite
						...
						//int err=INVITE(userbase, strtok(msg->data, ","), atoi(strtok(NULL,"\0")));
